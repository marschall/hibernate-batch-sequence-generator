...
import com.github.marschall.hibernate.batchsequencegenerator.HsqlTest.LocalTransactionManagerConfiguration;
import com.github.marschall.hibernate.batchsequencegenerator.configurations.HsqlConfiguration;

@Transactional
@ContextConfiguration(classes = {HsqlConfiguration.class, LocalTransactionManagerConfiguration.class})
//@Ignore
public class HsqlTest {
  ...
  private static final String S = "WITH RECURSIVE t(n, level_num) AS ("
          + "SELECT next value for seq_parent_id as n, 1 as level_num FROM (VALUES(0)) "
          + "UNION ALL "
  ...
  private static final String S_O =
          "WITH RECURSIVE "
+ " t(level_num) "
+ " AS "
+ " ( "
+ "     SELECT 1 as level_num FROM (VALUES(0)) "
+ "  "
+ "     UNION ALL "
+ "  "
+ "     SELECT "
...
//      try (ResultSet resultSet = statement.executeQuery("SELECT next value for seq_parent_id as n, 1 as level_num  FROM (VALUES(0))")) {
  ...
  @Test
  public void stackoverflow() throws SQLException {
    try (Connection connection = this.dataSource.getConnection();
            Statement statement = connection.createStatement()) {
      try (ResultSet resultSet = statement.executeQuery(S_O)) {
        while (resultSet.next()) {
          System.out.println(resultSet.getLong(1));
        }
      }
